...
class OnlineExpenseScreen extends StatefulWidget {
  ...
  const OnlineExpenseScreen({super.key, required this.selectedDate});

  @override
  State<OnlineExpenseScreen> createState() => _OnlineExpenseScreenState();
}

class _OnlineExpenseScreenState extends State<OnlineExpenseScreen> {
  final List<OnlineExpenseRow> _expenses = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final expenses = await _dbService.getOnlineExpenses(widget.selectedDate, branch.id);
      ...
      debugPrint('Error loading online expenses: $e');
      _addNewRow();
    } finally {
      setState(() {
        _isLoading = false;
  ...
  Future<void> _removeRow(int index) async {
    ...
    if (expense.id != null) {
      try {
        await _dbService.deleteOnlineExpense(expense.id!);
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      for (var expenseId in _existingExpenseIds) {
        try {
          await _dbService.deleteOnlineExpense(expenseId);
        } catch (e) {
      ...
      for (var expenseRow in _expenses) {
        if (expenseRow.amount != null && expenseRow.amount! > 0) {
          ...
          await _dbService.saveOnlineExpense(expense);
        }
      }
